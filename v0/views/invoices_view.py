...
class InvoicesView(QWidget):
    ...
    def __init__(self, db_manager, translation_manager, invoices_controller, clients_controller, appointments_controller, services_controller, is_admin):
        super().__init__()
        self.db_manager = db_manager
        self.translation_manager = translation_manager
        self.invoices_controller = invoices_controller
    ...
    def setup_ui(self):
        ...
        try:
            self.create_button.setIcon(QIcon(load_icon("add.png")))
        ...
        try:
            self.print_button.setIcon(QIcon(load_icon("print.png")))
        except FileNotFoundError:
            print("Warning: print.png icon not found")
        
        # Add buttons layout to main layout
        layout.addLayout(buttons_layout)
    
    def load_invoices(self):
        invoices = self.invoices_controller.get_all_invoices()
    ...
    def search(self, text):
        ...
        invoices = self.invoices_controller.search_invoices(text)
    ...
    def delete_invoice(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                self.invoices_controller.delete_invoice(invoice_id)
                self.load_invoices()
    ...
    def print_invoice(self):
        ...
        # Get invoice data
        invoice = self.invoices_controller.get_invoice(invoice_id)
        ...
        for i, service in enumerate(invoice["services"]):
            html += f"""
                <tr>
                    <td>{i+1}</td>
                    <td>{service["name"]}</td>
                    <td>{service["price"]:,.0f} {self.tr("services.price_currency")}</td>
                </tr>
            """
...
class InvoiceDialog(QDialog):
    """Dialog for adding, editing, or viewing an invoice."""
    
    def __init__(self, db_manager, translation_manager, invoices_controller, clients_controller, appointments_controller, services_controller, invoice_id=None, view_only=False):
        super().__init__()
        self.db_manager = db_manager
        self.translation_manager = translation_manager
        self.invoices_controller = invoices_controller
    ...
    def setup_ui(self):
        ...
        self.amount_remaining_spin = QDoubleSpinBox()
    ...
    def load_customers(self):
        customers = self.clients_controller.get_all_clients()
    ...
    def load_appointments_for_customer(self, customer_id):
        ...
        appointments = self.appointments_controller.get_appointments_by_customer(customer_id)
    ...
    def load_services(self):
        services = self.services_controller.get_all_services()
    ...
    def update_service_price(self):
        ...
        for row in self.service_rows:
            if row["combo"] == sender:
                ...
                if service_id:
                    service = self.services_controller.get_service(service_id)
    ...
    def update_remaining_amount(self):
        ...
        self.amount_remaining_spin.setValue(remaining)
    ...
    def appointment_changed(self):
        ...
        appointment = self.appointments_controller.get_appointment(appointment_id)
    ...
    def save_invoice(self):
        ...
        # Collect services
        services = []
        all_services = self.services_controller.get_all_services()
        for row in self.service_rows:
            if row["combo"].currentIndex() > 0:
                service_id = row["combo"].currentData()
                quantity = row["quantity"].value()
        ...
        try:
            if self.invoice_id:
                ...
                self.invoices_controller.update_invoice(self.invoice_id, invoice_data)
            else:
                # Add new invoice
                self.invoices_controller.add_invoice(invoice_data)
