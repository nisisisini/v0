...
class SettingsView(QDialog):
    ...
    def __init__(self, db_manager, settings, translation_manager, backup_manager, is_admin):
        super().__init__()
    ...
    def load_settings(self):
        # Load clinic info
        self.clinic_name_edit.setText(self.settings.get_setting("clinic_info.name", "مركز جوزيل للتجميل"))
        self.clinic_phone_edit.setText(self.settings.get_setting("clinic_info.phone", "+963956961395"))
        self.clinic_address_edit.setText(self.settings.get_setting("clinic_info.address", "سوريا - ريف دمشق التل موقف طيبة مقابل امركز الثقافي الجديد"))
        self.clinic_email_edit.setText(self.settings.get_setting("clinic_info.email", ""))
        
        # Load notification settings
        self.appointment_reminder_check.setChecked(self.settings.get_setting("notifications.appointment_reminder", True))
        self.reminder_hours_spin.setValue(self.settings.get_setting("notifications.reminder_hours_before", 24))
        
        # Load backup settings
        self.auto_backup_check.setChecked(self.settings.get_setting("backup.auto_backup", True))
        self.backup_interval_spin.setValue(self.settings.get_setting("backup.backup_interval_days", 1))
        self.backup_location_edit.setText(self.settings.get_setting("backup.backup_location", "backups/"))
    
    def save_settings(self):
        # Save clinic info
        self.settings.set_setting("clinic_info.name", self.clinic_name_edit.text())
        self.settings.set_setting("clinic_info.phone", self.clinic_phone_edit.text())
        self.settings.set_setting("clinic_info.address", self.clinic_address_edit.text())
        self.settings.set_setting("clinic_info.email", self.clinic_email_edit.text())
        
        # Save notification settings
        self.settings.set_setting("notifications.appointment_reminder", self.appointment_reminder_check.isChecked())
        self.settings.set_setting("notifications.reminder_hours_before", self.reminder_hours_spin.value())
        
        # Save backup settings
        self.settings.set_setting("backup.auto_backup", self.auto_backup_check.isChecked())
        self.settings.set_setting("backup.backup_interval_days", self.backup_interval_spin.value())
        self.settings.set_setting("backup.backup_location", self.backup_location_edit.text())
    ...
    def load_users(self):
        ...
        auth_controller = AuthController(self.db_manager)
        ...
        for i, user in enumerate(users):
            ...
            self.users_table.setItem(i, 1, QTableWidgetItem("✓" if user["is_admin"] else ""))
            ...
            self.users_table.setCellWidget(i, 2, actions_widget)
    
    def add_user(self):
        dialog = UserDialog(self.db_manager, self.translation_manager)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.load_users()
    
    def edit_user(self, username):
        dialog = UserDialog(self.db_manager, self.translation_manager, username)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.load_users()
    ...
    def delete_user(self, username):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if auth_controller.delete_user(username):
                QMessageBox.information(self, self.tr("common.success"), self.tr("user_deleted"))
                self.load_users()
            else:
                QMessageBox.critical(self, self.tr("common.error"), self.tr("delete_failed"))


class UserDialog(QDialog):
    """Dialog for adding or editing a user."""
    
    def __init__(self, db_manager, translation_manager, username=None):
        super().__init__()
        self.db_manager = db_manager
        self.translation_manager = translation_manager
        self.tr = self.translation_manager.get_translation
    ...
    def load_user(self, username):
        ...
        if user:
            ...
            self.is_admin_check.setChecked(user["is_admin"])
    ...
    def save_user(self):
        ...
        try:
            if self.username:
                ...
                success = auth_controller.update_user(
                    self.username,
                    new_password=self.password_edit.text() if self.password_edit.text() else None,
                    is_admin=self.is_admin_check.isChecked()
                )
            else:
                # Add new user
                success = auth_controller.add_user(
                    self.username_edit.text(),
                    self.password_edit.text(),
                    self.is_admin_check.isChecked()
                )
            
            if success:
                self.accept()
            else:
                QMessageBox.critical(self, self.tr("common.error"), self.tr("operation_failed"))
        except Exception as e:
            QMessageBox.critical(self, self.tr("common.error"), f"{self.tr('operation_failed')}: {str(e)}")
